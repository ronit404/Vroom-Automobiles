...
<head>
    ...
    <title>Manage Automobiles</title>
...
<body class="min-h-screen flex flex-col" style="background: linear-gradient(to right, #dfe9f3 0%, #ffffff 100%);">
    ...
    <div class="flex justify-center mt-10 px-4">
        <div class="bg-white shadow-2xl rounded-2xl p-8 w-full max-w-md border border-gray-200">
            ...
            <h2 class="text-3xl font-bold text-center text-gray-800 mb-6">Manage Automobiles</h2>

            <!-- Form to Add Automobile -->
            <form action="../php/add_automobile.php" method="POST" class="space-y-5">
                ...
                <input type="number" name="price" placeholder="Price" required
                    class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500">
                <button type="submit"
                        class="w-full py-3 text-white bg-green-500 rounded-xl hover:bg-green-600 transition duration-200 font-semibold">Add Automobile</button>
            </form>

            <!-- List of Automobiles -->
            <h3 class="text-2xl font-semibold text-gray-800 mt-8 mb-4">Available Automobiles</h3>
            <ul class="space-y-3">
                <?php foreach ($autos as $auto): ?>
                    <li class="flex justify-between items-center bg-gray-100 p-3 rounded-lg shadow-sm hover:bg-gray-200 transition">
                        <span class="text-gray-800"><?php echo htmlspecialchars($auto['brand']) . " " . htmlspecialchars($auto['model']) . " - ₹" . number_format($auto['price']); ?></span>
                        <a href="../php/delete_automobile.php?id=<?php echo $auto['id']; ?>" class="text-red-500 hover:text-red-600 font-semibold">Delete</a>
                    </li>
                <?php endforeach; ?>
            </ul>
